...
public class BookServiceImpl implements BookService {
	...
	@Autowired    
	private BookRedisRepository bookRedisRepo;
	...
	public Book getBookById(String bookId) {
		...
		// always look in cache first
		Book book = this.checkRedisCache(bookId);
		...
		if (book != null) {	
			...
			try {
				...
				this.cacheBookObject(book);// save to Redis
	...
	private Book checkRedisCache(String bookId) {
        System.out.println("checkRedisCache begin");
    	try {
            return bookRedisRepo.findBookById(bookId);
        }
        catch (Exception ex){
	...
	private void cacheBookObject(Book book) {	
		System.out.println("cacheBookObject begin");    
	    try {
	    	bookRedisRepo.saveBook(book);
	    }catch (Exception ex){
	    	System.out.println("Exception caught " 
	            					+ book.getId() + " " + ex);    
	    }
